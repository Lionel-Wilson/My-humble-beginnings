...
class Hero:
    ...
    def healing(self):
        if (self.health+self.heal)<=self.max_health:
            if self.energy>=self.heal_energy:
                ...
                print(f"{self.name} used '{self.heal_move}' and restored {self.heal}HP!")
                return
            else:
            ...
            if self.energy>=self.heal_energy:
                ...
                print(f"{self.name} used '{self.heal_move}' and maxed out their HP")
                return
            else:
                print(f"\n{self.name} tried to use '{self.heal_move}' but didn't have enough energy. {self.name}'s guard is now OPEN!")
                return
    ...
    def battle_complete_screen(self):
        global critical_counter
        max_xp_increase = 500
        hp_multiplier = 5
        crit_multiplier = 20
...
def startgame():
    ...
    input(f"\n{hero.name}: *sigh*")
    input(f"\n{hero.name}: *Pulls out knife from back pouch and sprints towards half dead angel*")
...
class Enemy:
    def __init__(self,name="None",class_name="none",level = 1,na_dmg=5,sa_dmg=15):
        self.health = 100
        self.level = level
        self.name = name
...
Angel = Angel("Angel","Angel",3,7,14)
Angel.normal_attack = "Light Ray"
Angel.special_attack = "Eternal Purification"
